...
def game(level, model):
    ...
    while running:
        ...
        # input image au model pour faire la prediction (move contient la prediction - 'up', 'down' etc)
        move = keras_model.pred(model)
        
        
        if move == 'up' :#and directions[-1] != "down" and directions[-1] != "up":
            print('action up')
            x_change = 0
            y_change = -10
        ...
        #move down    
        elif move == 'down': #and directions[-1] != "up" and directions[-1] != "down":
            print('action down', move)
            x_change = 0
            y_change = 10
        ...
        #move right
        elif move == 'right': #and directions[-1] != "left" and directions[-1] != "right":
            print('action right')
            x_change = 10
            y_change =0
        ...
        #move left
        elif move == 'left': #and directions[-1] != "right" and directions[-1] != "left":
            print('action left')
            x_change = -10
            y_change = 0
...
def end_game(level):
    model = tensorflow.keras.models.load_model('converted_keras_final/keras_model.h5', compile=False)
    wait = True
    while wait:
